...
func RegisterAdminRoutes(engine *gin.Engine, db *gorm.DB) {
	...
	noticesService := services.NewNoticesService(noticesRepository)
	...
	groupapi_v1 := engine.Group("/api/v1")
	{
		...
		groupapi_v1.POST("/users", middleware.JWT(), middleware.RBAC("sys:user:create"), userController.CreateUser)
